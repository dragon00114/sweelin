...
$(window).scroll(function () {
	...
	if (windowScrollTop >= $('#machine').offset().top + Math.min(windowHeight, windowWidht)) {
...
function media() {
	...
	if (w < h) {
	  offsetPerSlide = 200;
	  $('.home, #card, #sweelin, #nav, .bar_wrapper, #vending').addClass('horizontal');

	  $('#vending .vending_wrapper').html('').append(imgElements_mob);
	  $('#nav-vertical > *').detach().prependTo('#nav-horizontal');
	} else {
		offsetPerSlide = 160;
		$('.home, #card, #sweelin, #nav, .bar_wrapper, #vending').removeClass('horizontal');
...
function vending(el, height, top) {
  ...
  for (var i = 0; i <= count; i++) {
    ...
    imgElements.push($(`<img style="display: none;" src="/images/vending/${name}">`));

    if (i <= 25) {
    	imgElements_mob.push($(`<img style="display: none;" src="/images/vendingMob/${name}">`));
    } else {
    	imgElements_mob.push($(`<img style="display: none;" src="/images/vending/${name}">`));
    }
  }
...
function sliderStart(activeIndex) {
	...
	if ($('.machine .machine_active').length) {
		...
		// if ($(window).width() < $(window).height()) {
		// 	activeSlide.allowTouchMove = true;
		// } else {
		// 	activeSlide.allowTouchMove = false;
		// }
	}
...
function knotEmoj(index) {
	$('#emoj').find('.emoj_img').eq(index).addClass(a)
	...
	$(window).scroll(function () {
		...
		if (windowScrollTop >= $('#card').offset().top && windowScrollTop <= $('#card-end').offset().top  - windowHeight) {
			...
			if (windowHeight > $(window).width()) {
				...
				var posTop = $('.card_wrapepr').outerHeight() - $(window).width() * .5  - windowHeight * 1.6;
			}
...
function sweelin_scroll() {
	$(window).scroll(function () {
		...
		if (windowScrollTop >= $('#how').offset().top + windowHeight * .8) {
			$('#how').addClass(a);
		} else {
			$('#how').removeClass(a);
		}
...
function sweelin_lax() {
  ...
  var how = $('#how'),
	...
	lax.addElements('.how_sugar_spoon', {
	  scrollY: {
	  	scale: [
	  		[how_top - windowHeight * .2, how_top + windowHeight * .4],
	    	['.6', '1']
	  	],
	  	rotate: [
	  		[how_top - windowHeight * .2, how_top + windowHeight * .4],
...
function form_send(i) {
	...
	$.ajax({
		...
		success: function(html) {
			...
			if (json.process == 1) {
				input_name.val('').removeClass(a).siblings('.input_palceholder').removeClass(a);
				input_email.val('').removeClass(a).siblings('.input_palceholder').removeClass(a);
				input_message.val('').removeClass(a).siblings('.input_palceholder').removeClass(a);
				input_updates.prop('checked', false);
				$('#interested option[value=placeholder]').prop('selected', true);
				$('#interested').siblings('.current').text($('#interested option[value=placeholder]').text());
				$('#interested').siblings('.select_dropdown').find('li').removeClass('selected');

				setTimeout(function() {
					$('.form_btn').hide().before('<div class="form_bottom_message" id="message">your message has been sent</div>');
				}, 10);
			} else {
				if (json.errs.name) {
    ...
    function animOnScroll() {
      for (let index = 0; index < animItems.length; index++) {
        ...
        if (animItem.classList.contains('_anim-center')) {
          ...
          if (windowCenter >= animItemCenter - animItemHeight / 2 && windowCenter <= animItemCenter + animItemHeight / 2) {
            animItem.classList.add('_anim_act');
          }
        } else if (animItem.classList.contains('_anim-top')) {
          if (pageYOffset >= animItemOffset - 1 && pageYOffset <= animItemOffset + animItemHeight) {
            animItem.classList.add('_anim_act');
          }
        } else {
